...
async def search(
    ...
    perplexity_response = await _PERPLEXITY_SERVICE.search(agent_name=agent_name, query=query, recency=recency)
...
async def deep_search(
    ...
    if recency is not None and recency not in ['day', 'week', 'month', 'year']:
        return {"error": "Invalid recency. Must be 'day', 'week', 'month', or 'year' or leave it blank."}
    perplexity_response = await _PERPLEXITY_SERVICE.deep_search(agent_name=agent_name, query=query, recency=recency)
    return perplexity_response.model_dump()
